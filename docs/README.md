# Documentação

Documentamos o que **não** é óbvio no código — decisões e arquitetura. O resto
tem fonte única: contrato vem do OpenAPI, fronteiras dos módulos do Spring
Modulith, setup e padrões do CONTRIBUTING.

| Preciso de... | Onde |
|---|---|
| Rodar o projeto | [README.md](../README.md) |
| Contribuir, padrões, setup, testes | [CONTRIBUTING.md](../CONTRIBUTING.md) |
| Por que decidimos X | [adr/](adr/) |
| Como os módulos se encaixam | [architecture/c4.md](architecture/c4.md) |
| Endpoints e schemas | [Swagger UI](http://localhost:8080/docs) · [OpenAPI JSON](http://localhost:8080/v3/api-docs) |
| Chamar a API na prática | [collection Bruno](../bruno) |

