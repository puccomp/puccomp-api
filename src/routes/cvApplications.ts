...
import { createUpload, sanitizeFileName } from '../utils/uploads.js'
...
router.post(
  ...
  (async (req, res) => {
    ...
    if (!fullName?.trim() || !phone?.trim() || !course?.trim() || !period?.trim()) {
      res
        .status(400)
        .json({ message: 'fullName, phone, course, and period are required' })
      return
    }
