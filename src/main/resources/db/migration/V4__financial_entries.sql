create table financial_entries (
    ...
    primary key (id),
    constraint ck_financial_entries_value_positive check (value > 0)
);

create index ix_financial_entries_tenant_date
    on financial_entries (tenant_id, date desc);

create index ix_financial_entries_tenant_type_date
    on financial_entries (tenant_id, type, date desc);

-- Registra FINANCIAL_READ e FINANCIAL_WRITE nas check constraints das tabelas de permissão.
-- OWNER já recebe todas as permissões pelo catálogo em código.
